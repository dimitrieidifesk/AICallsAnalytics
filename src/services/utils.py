...
async def process_audio_recording(call_session_id: uuid.UUID) -> None:
    async for session in db_connector.session_getter():
        call_session_repo = CallSessionRepository(session)
        call_session = await call_session_repo.get_call_session_by_id(call_session_id)
        if not call_session:
            logger.error(f"Call session id {call_session_id} not found!")
            return

        await call_session_repo.update_call_session(call_session_id, {"status": CallSessionStatus.IN_PROCESSING})
        ...
        if not text_from_audio:
            ...
            await call_session_repo.update_call_session(
                call_session_id, {"status": CallSessionStatus.ERROR_RECEIVING_TRANSCRIPTION}
            )
            return

        await call_session_repo.update_call_session(call_session_id, {"text_from_audio": text_from_audio})
        data = await open_ai_service.structure_text_with_chatgpt(text_from_audio)
        try:
            structure_text = json.loads(data["choices"][0]["message"]["content"])
            await call_session_repo.update_call_session(
                call_session_id, {"transcription": {"data": structure_text}})
        except Exception as e:
            logger.error(f"Structure text from url failed! Error: {e}")
            data = await open_ai_service.structure_text_with_chatgpt(text_from_audio)
            structure_text = json.loads(data["choices"][0]["message"]["content"])
            await call_session_repo.update_call_session(
                call_session_id, {"transcription": {"data": structure_text}})
